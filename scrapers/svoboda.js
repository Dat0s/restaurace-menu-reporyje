      ...
      .evaluate(async () => {
        try {
          ...
          const res = await fetch(
            "/api/v1/users/web_profile_info/?username=svoboda_reznictvi",
            {
              headers: {
          ...
          if (edges && edges.length > 0) {
            return edges
              .map((e) => e.node.display_url || e.node.thumbnail_src || "")
              .filter(Boolean);
          }
        } catch {}
        return [];
    ...
    if (imageUrls.length === 0) {
      imageUrls = await page.evaluate(() => {
        const imgs = Array.from(
          document.querySelectorAll("article img, main img"),
        );
        return imgs
          .filter((img) => {
            const src = img.src || "";
...
function parseMenuText(text) {
  ...
  // Extract date range (e.g. "16.3. - 20.3. KW 4" or "1.6- 5.6. K4")
  let menuDate = "";
