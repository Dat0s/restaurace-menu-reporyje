  ...
  try {
    ...
    const imageUrl = await page.evaluate(() => {
      ...
      const otherImgs = allSources
        ...
        .filter(
          (src) =>
            src.includes("res2.weblium.site") &&
            !src.includes(staticPrefix) &&
            src.length > 20,
        );
...
async function scrapeStaticMenu() {
  try {
    ...
    for (const def of sectionDefs) {
      ...
      const itemPattern =
        /([A-ZÁ-Ža-zá-ž][A-ZÁ-Ža-zá-ž0-9\s\/\(\),–—.čřžšďťňůúýáéíó]+?)\s*(\d+,-(?:\/\d+,-)?)/g;
      let match;
      const items = [];
      while ((match = itemPattern.exec(sectionText)) !== null) {
        ...
        if (
          /Nenechte|Našim|Máte chuť|Čekají|nabízíme|ochutnat|Položky v tomto/i.test(
            name,
          )
        )
          continue;
